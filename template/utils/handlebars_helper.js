define([
     ...
     *
     */
    Handlebars.registerHelper('nl2brAndtoLowerCase', function(text) {
        return _handlebarsNewlineToBreak(text).toLowerCase();
    });

    /**
     *
     */
    Handlebars.registerHelper('paramsArray', function(text) {
      let a = (/^(.*)\[\]$/.test(text)) ?
      `array of ${text.slice(0, -2)}` :
      text;
     ...
     *
     */
    Handlebars.registerHelper('rmColon', function(text) {
      return text.replace(':', '');
    });
     ...
     *
     */
    Handlebars.registerHelper('splitAllow', function(allow) {
        allow = allow.join(', ')
        return allow
    });
